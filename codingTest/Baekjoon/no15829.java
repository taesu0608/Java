...
보통 r과 M은 서로소인 숫자로 정하는 것이 일반적이다. 우리가 직접 정하라고 하면 힘들테니까 r의 값은 26보다
큰 소수인 31로 하고 M의 값은 1234567891(놀랍게도 소수이다!!)로 하자.
...
public class no15829 {
    private static final BigInteger MOD = BigInteger.valueOf(1234567891L);
    private static final BigInteger BASE = BigInteger.valueOf(31L);

    public static void main(String[] args) throws IOException {
        BufferedReader br = new BufferedReader(new InputStreamReader(System.in));
        int n = Integer.parseInt(br.readLine()); // 길이 (사실 input.length()와 같음)
        ...
        for (int i = 0; i < input.length(); i++) {
            int val = input.charAt(i) - 'a' + 1; // 1~26

            // result += val * r (mod MOD)
            result = result.add(BigInteger.valueOf(val).multiply(r)).mod(MOD);
